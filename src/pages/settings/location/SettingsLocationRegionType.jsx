...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationRegionType = () => {
  ...
  let tableName = SettingsLocationRegionTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let regionTypes = useSelector((state) => state.settings.regionTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationRegionType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getRegionTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationRegionTypeUrl, null, null);
      dispatch(getRegionTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    regionTypes && regionTypes.length > 0 ? setData(regionTypes) : setData([]);

    setLoading(false);
  }, [regionTypes, isLoading, loading]);
...
export default SettingsLocationRegionType;
