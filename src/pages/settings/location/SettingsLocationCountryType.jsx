...
import { getCountryTypes } from "../../../redux/slice";
...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationCountryType = () => {
  ...
  let tableName = SettingsLocationCountryTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let countryTypes = useSelector((state) => state.settings.countryTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationCountryType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getCountryTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationCountryTypeUrl, null, null);
      dispatch(getCountryTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    countryTypes && countryTypes.length > 0
      ? setData(countryTypes)
      : setData([]);

    setLoading(false);
  }, [countryTypes, isLoading, loading]);
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationCountryType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationCountryType;
