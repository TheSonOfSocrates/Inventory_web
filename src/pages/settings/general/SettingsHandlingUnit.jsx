...
const SettingsHandlingUnit = () => {
	...
	let tableName = SettingsHandlingUnitUrl.split("/")
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsHandlingUnit/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getHandlingUnits(res));
		} else {
			let res = await CRUD("get", SettingsHandlingUnitUrl, null, null);
			dispatch(getHandlingUnits(res));
		}
	};
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsHandlingUnitUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsHandlingUnitUrl, null, null);
					dispatch(getHandlingUnits(res));
				} else return result;
			} else {
				let result = await CRUD("post", SettingsHandlingUnitUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsHandlingUnitUrl, null, null);
					dispatch(getHandlingUnits(res));
				}
	...
	const getItem = useCallback(async () => {
		...
		handlingUnits && handlingUnits.length > 0
			? setData(handlingUnits)
			: setData([]);
		// UOMs && UOMs.length > 0 ? setUOMs(UOMs) : setData([]);
		setLoading(false);
	}, [UOMs, handlingUnits, isLoading, loading]);
	...
	const columns = [
		...
		},
		{
			...
			dataIndex: "weight",
			key: "weight",
			sorter: defaultColumnSorter("weight"),
			sortDirections: ["descend", "ascend"],
		},
		{
			title: "UomId",
...
export default SettingsHandlingUnit;
