...
const SettingsReasonsType = () => {
  ...
  let tableName = SettingsReasonsTypeUrl.split("/")
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsReasonsType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getReasonsTypes(res));
    } else {
      let res = await CRUD("get", SettingsReasonsTypeUrl, null, null);
      dispatch(getReasonsTypes(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsReasonsTypeUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsReasonsTypeUrl, null, null);
          dispatch(getReasonsTypes(res));
        }
      } else {
        let result = await CRUD("post", SettingsReasonsTypeUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsReasonsTypeUrl, null, null);
          dispatch(getReasonsTypes(res));
        }
  ...
  const getItem = useCallback(async () => {
    ...
    reasonsTypes && reasonsTypes.length > 0
      ? setData(reasonsTypes)
      : setData([]);
    setLoading(false);
  }, [reasonsTypes, isLoading, loading]);
  ...
  const processDataForExcel = () => {
    return data.map((record) => {
      ...
      let selReasonType = reasonsTypes.filter(
        (item) => item.id === record.type_id
      );      
...
export default SettingsReasonsType;
