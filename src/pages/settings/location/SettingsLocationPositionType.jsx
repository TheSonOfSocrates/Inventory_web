...
import { getPositionTypes } from "../../../redux/slice";
...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationPositionType = () => {
  ...
  let tableName = SettingsLocationPositionTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let positionTypes = useSelector((state) => state.settings.positionTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationPositionType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getPositionTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationPositionTypeUrl, null, null);
      dispatch(getPositionTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    positionTypes && positionTypes.length > 0
      ? setData(positionTypes)
      : setData([]);

    setLoading(false);
  }, [positionTypes, isLoading, loading]);
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: defaultColumnSorter("defaultColumnSorter"),
      sortDirections: ["descend", "ascend"],
      render: (_, record, index) => {
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationPositionType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationPositionType;
