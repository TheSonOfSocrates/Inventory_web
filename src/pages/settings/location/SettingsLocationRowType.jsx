...
const SettingsLocationRowType = () => {
  ...
  let tableName = SettingsLocationRowTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let rowTypes = useSelector((state) => state.settings.rowTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationRowType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getRowTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationRowTypeUrl, null, null);
      dispatch(getRowTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    rowTypes && rowTypes.length > 0 ? setData(rowTypes) : setData([]);

    setLoading(false);
  }, [rowTypes, isLoading, loading]);
...
export default SettingsLocationRowType;
