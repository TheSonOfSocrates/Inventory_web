...
const SettingsLocationBinType = () => {
  ...
  let tableName = SettingsLocationBinTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let binTypes = useSelector((state) => state.settings.binTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationBinType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getBinTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationBinTypeUrl, null, null);
      dispatch(getBinTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    binTypes && binTypes.length > 0 ? setData(binTypes) : setData([]);

    setLoading(false);
  }, [binTypes, isLoading, loading]);
...
export default SettingsLocationBinType;
