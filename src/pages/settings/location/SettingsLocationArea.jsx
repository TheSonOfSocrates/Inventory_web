...
const SettingsLocationArea = () => {
	...
	let tableName = SettingsLocationAreaUrl.split("/")
	...
	let initTypes = useSelector((state) => state.settings.areaTypes);
	let initParentLocations = useSelector((state) => state.settings.zones);
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsLocationArea/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getAreas(res));
		} else {
			let res = await CRUD("get", SettingsLocationAreaUrl, null, null);
			dispatch(getAreas(res));
		}
	};
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsLocationAreaUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsLocationAreaUrl, null, null);
					dispatch(getAreas(res));
				}
			} else {
				let result = await CRUD("post", SettingsLocationAreaUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsLocationAreaUrl, null, null);
					dispatch(getAreas(res));
				}
	...
	const handleDelete = async (record) => {
		let result = await CRUD("delete", SettingsLocationAreaUrl, null, record.id);
		if (result) {
			let res = await CRUD("get", SettingsLocationAreaUrl, null, null);
			dispatch(getAreas(res));
		}
	...
	const getItem = useCallback(async () => {
		...
		initReports && initReports.length > 0
			...
			: setReports([]);
		areas && areas.length > 0 ? setData(areas) : setData([]);
		setLoading(false);
	}, [isLoading, loading, uoms, initTypes, initParentLocations, initReports, areas, dispatch]);
	...
	const columns = [
		...
		},
		{
			...
			sortDirections: ["descend", "ascend"],
			render: (_, record, index) => {
				let r1 = types.filter((item) => item.id === record.type_id);
		...
		},
		{
			...
			sortDirections: ["descend", "ascend"],
			render: (_, record, index) => {
				let r1 = reports.filter((item) => item.id === record.report_id);
				return r1.length > 0 ? r1[0].name : "";
			},
		},
		{
...
export default SettingsLocationArea;
