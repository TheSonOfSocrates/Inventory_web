...
const SettingsUOM = () => {
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl + `api/SettingsUOM/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getUOMs(res));
		} else {
			let res = await CRUD("get", SettingsUOMUrl, null, null);
			dispatch(getUOMs(res));
		}
	};
	...
	let tableName = SettingsUOMUrl.split("/")
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsUOMUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsUOMUrl, null, null);
					dispatch(getUOMs(res));
				}
			} else {
				let result = await CRUD("post", SettingsUOMUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsUOMUrl, null, null);
					dispatch(getUOMs(res));
				}
	...
	const handleDelete = async (record) => {
		let result = await CRUD("delete", SettingsUOMUrl, record, record.id);
		if (result) {
			let res = await CRUD("get", SettingsUOMUrl, null, null);
			dispatch(getUOMs(res));
		}
	...
	const getItem = useCallback(async () => {
		...
		UOMs && UOMs.length > 0 ? setData(UOMs) : setData([]);
		setLoading(false);
	}, [UOMs, isLoading, loading]);
	...
	const columns = [
		...
		},
		{
			...
			dataIndex: "acronym",
			key: "acronym",
			sorter: defaultColumnSorter("acronym"),
			sortDirections: ["descend", "ascend"],
		},
		{
