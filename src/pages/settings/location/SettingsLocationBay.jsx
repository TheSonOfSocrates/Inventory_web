...
const SettingsLocationBay = () => {
	...
	let tableName = SettingsLocationBayUrl.split("/")
	...
	let initTypes = useSelector((state) => state.settings.bayTypes);
	let initParentLocations = useSelector((state) => state.settings.rows);
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsLocationBay/` +
				`?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getBays(res));
		} else {
			let res = await CRUD("get", SettingsLocationBayUrl, null, null);
			dispatch(getBays(res));
		}
	};
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsLocationBayUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsLocationBayUrl, null, null);
					dispatch(getBays(res));
				}
			} else {
				let result = await CRUD("post", SettingsLocationBayUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsLocationBayUrl, null, null);
					dispatch(getBays(res));
				}
	...
	const handleDelete = async (record) => {
		let result = await CRUD("delete", SettingsLocationBayUrl, null, record.id);
		if (result) {
			let res = await CRUD("get", SettingsLocationBayUrl, null, null);
			dispatch(getBays(res));
		}
	...
	const getItem = useCallback(async () => {
		...
		bays && bays.length > 0 ? setData(bays) : setData([]);
