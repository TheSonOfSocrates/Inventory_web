...
const SettingsCurrency = () => {
  ...
  let tableName = SettingsCurrencyUrl.split("/")
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsCurrency/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getCurrencies(res));
    } else {
      let res = await CRUD("get", SettingsCurrencyUrl, null, null);
      dispatch(getCurrencies(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsCurrencyUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsCurrencyUrl, null, null);
          dispatch(getCurrencies(res));
        } else return result;
      } else {
        let result = await CRUD("post", SettingsCurrencyUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsCurrencyUrl, null, null);
          dispatch(getCurrencies(res));
        } else return result;
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsCurrencyUrl, record, record.id);
    if (result) {
      let res = await CRUD("get", SettingsCurrencyUrl, null, null);
      dispatch(getCurrencies(res));
    } else return result;
  ...
  const getItem = useCallback(async () => {
    ...
    currencies && currencies.length > 0 ? setData(currencies) : setData([]);
    setLoading(false);
  }, [currencies, isLoading, loading]);
