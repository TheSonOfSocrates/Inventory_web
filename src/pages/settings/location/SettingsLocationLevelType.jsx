...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationLevelType = () => {
  ...
  let tableName = SettingsLocationLevelTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let levelTypes = useSelector((state) => state.settings.levelTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationLevelType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getLevelTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationLevelTypeUrl, null, null);
      dispatch(getLevelTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    levelTypes && levelTypes.length > 0 ? setData(levelTypes) : setData([]);

    setLoading(false);
  }, [levelTypes, isLoading, loading]);
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationLevelType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationLevelType;
