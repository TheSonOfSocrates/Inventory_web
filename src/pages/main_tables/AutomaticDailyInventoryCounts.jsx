...
import {
  AutomaticDailyInventoryCountsUrl,
...
const { Search } = Input;
const { Option } = Select;
const AutomaticDailyInventoryCounts = () => {
  ...
  let tableName = AutomaticDailyInventoryCountsUrl.split("/")
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/AutomaticDailyInventoryCountsUrl/?timestamp=${timestamp}&search=${value}`;
  ...
  const handleDelete = async (record) => {
    ...
    if (result) {
      let res = await CRUD("get", AutomaticDailyInventoryCountsUrl, null, null);
      setData(res);
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "code",
      render: (_, record, index) => {
        let r1 = currentItemInventories.filter(
...
export default AutomaticDailyInventoryCounts;
