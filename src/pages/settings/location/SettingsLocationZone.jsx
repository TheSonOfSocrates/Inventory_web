...
const SettingsLocationZone = () => {
	...
	let tableName = SettingsLocationZoneUrl.split("/")
	...
	let initTypes = useSelector((state) => state.settings.zoneTypes);
	let initParentLocations = useSelector((state) => state.settings.warehouses);
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsLocationZone/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getZones(res));
		} else {
			let res = await CRUD("get", SettingsLocationZoneUrl, null, null);
			dispatch(getZones(res));
		}
	};
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsLocationZoneUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsLocationZoneUrl, null, null);
					dispatch(getZones(res));
				}
			} else {
				let result = await CRUD("post", SettingsLocationZoneUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsLocationZoneUrl, null, null);
					dispatch(getZones(res));
				}
	...
	const handleDelete = async (record) => {
		let result = await CRUD("delete", SettingsLocationZoneUrl, null, record.id);
		if (result) {
			let res = await CRUD("get", SettingsLocationZoneUrl, null, null);
			dispatch(getZones(res));
		}
	...
	const getItem = useCallback(async () => {
		...
		zones && zones.length > 0 ? setData(zones) : setData([]);
		...
		<div>
			...
			>
				<Form form={form} {...formItemLayout}>
					...
					>
						<Select style={{ width: relative }}>
							{initParentLocations.map((item, index) => (
								<Option key={item.id} value={item.id}>
									{item.name}
								</Option>
							))}
...
export default SettingsLocationZone;
