...
const SettingsReports = () => {
  ...
  let tableName = SettingsReportsUrl.split("/")
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl + `api/SettingsReports/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getReports(res));
    } else {
      let res = await CRUD("get", SettingsReportsUrl, null, null);
      dispatch(getReports(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsReportsUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsReportsUrl, null, null);
          dispatch(getReports(res));
        }
      } else {
        let result = await CRUD("post", SettingsReportsUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsReportsUrl, null, null);
          dispatch(getReports(res));
        }
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsReportsUrl, record, record.id);
    if (result) {
      let res = await CRUD("get", SettingsReportsUrl, null, null);
      dispatch(getReports(res));
    }
  ...
  const getItem = useCallback(async () => {
    ...
    reports && reports.length > 0 ? setData(reports) : setData([]);
    setLoading(false);
