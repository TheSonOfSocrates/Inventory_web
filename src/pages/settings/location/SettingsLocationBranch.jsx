...
const SettingsLocationBranch = () => {
  ...
  let tableName = SettingsLocationBranchUrl.split("/")
  ...
  let initTypes = useSelector((state) => state.settings.branchTypes);
  let initParentLocations = useSelector((state) => state.settings.regions);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationBranch/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getBranches(res));
    } else {
      let res = await CRUD("get", SettingsLocationBranchUrl, null, null);
      dispatch(getBranches(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    branches && branches.length > 0 ? setData(branches) : setData([]);
...
export default SettingsLocationBranch;
