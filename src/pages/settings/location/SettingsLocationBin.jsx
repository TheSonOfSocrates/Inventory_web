...
const SettingsLocationBin = () => {
  ...
  let tableName = SettingsLocationBinUrl.split("/")
  ...
  let initTypes = useSelector((state) => state.settings.binTypes);
  let initParentLocations = useSelector((state) => state.settings.positions);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationBin/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getBins(res));
    } else {
      let res = await CRUD("get", SettingsLocationBinUrl, null, null);
      dispatch(getBins(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsLocationBinUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsLocationBinUrl, null, null);
          dispatch(getBins(res));
        }
      } else {
        let result = await CRUD("post", SettingsLocationBinUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsLocationBinUrl, null, null);
          dispatch(getBins(res));
        }
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsLocationBinUrl, null, record.id);
    if (result) {
      let res = await CRUD("get", SettingsLocationBinUrl, null, null);
      dispatch(getBins(res));
    }
  ...
  const getItem = useCallback(async () => {
    ...
    bins && bins.length > 0 ? setData(bins) : setData([]);
...
export default SettingsLocationBin;
