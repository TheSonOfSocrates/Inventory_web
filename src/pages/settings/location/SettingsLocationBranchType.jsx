...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationBranchType = () => {
  ...
  let tableName = SettingsLocationBranchTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let branchTypes = useSelector((state) => state.settings.branchTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationBranchType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getBranchTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationBranchTypeUrl, null, null);
      dispatch(getBranchTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    branchTypes && branchTypes.length > 0 ? setData(branchTypes) : setData([]);

    setLoading(false);
  }, [branchTypes, isLoading, loading]);
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationBranchType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationBranchType;
