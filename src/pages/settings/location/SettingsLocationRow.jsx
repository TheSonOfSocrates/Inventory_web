...
const SettingsLocationRow = () => {
  ...
  let tableName = SettingsLocationRowUrl.split("/")
  ...
  let initTypes = useSelector((state) => state.settings.rowTypes);
  let initParentLocations = useSelector((state) => state.settings.rooms);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationRow/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getRows(res));
    } else {
      let res = await CRUD("get", SettingsLocationRowUrl, null, null);
      dispatch(getRows(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsLocationRowUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsLocationRowUrl, null, null);
          dispatch(getRows(res));
        }
      } else {
        let result = await CRUD("post", SettingsLocationRowUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsLocationRowUrl, null, null);
          dispatch(getRows(res));
        }
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsLocationRowUrl, null, record.id);
    if (result) {
      let res = await CRUD("get", SettingsLocationRowUrl, null, null);
      dispatch(getRows(res));
    }
  ...
  const getItem = useCallback(async () => {
    ...
    rows && rows.length > 0 ? setData(rows) : setData([]);
...
export default SettingsLocationRow;
