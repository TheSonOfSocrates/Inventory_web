...
import { getWarehouseTypes } from "../../../redux/slice";
...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationWarehouseType = () => {
  ...
  let tableName = SettingsLocationWarehouseTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let warehouseTypes = useSelector((state) => state.settings.warehouseTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationWarehouseType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getWarehouseTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationWarehouseTypeUrl, null, null);
      dispatch(getWarehouseTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    warehouseTypes && warehouseTypes.length > 0
      ? setData(warehouseTypes)
      : setData([]);

    setLoading(false);
  }, [warehouseTypes, isLoading, loading]);
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationWarehouseType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationWarehouseType;
