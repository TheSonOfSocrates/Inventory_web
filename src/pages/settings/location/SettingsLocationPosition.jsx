...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationPosition = () => {
  ...
  let tableName = SettingsLocationPositionUrl.split("/")
  ...
  let initTypes = useSelector((state) => state.settings.positionTypes);
  let initParentLocations = useSelector((state) => state.settings.levels);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationPosition/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getPositions(res));
    } else {
      let res = await CRUD("get", SettingsLocationPositionUrl, null, null);
      dispatch(getPositions(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    positions && positions.length > 0 ? setData(positions) : setData([]);
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationPosition"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationPosition;
