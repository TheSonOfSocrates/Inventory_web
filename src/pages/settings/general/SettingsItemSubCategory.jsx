...
import { SettingsItemSubCategoryUrl, BaseUrl } from "../../../utils/network";
import { CRUD, search, customRequest } from "../../../utils/js_functions";
import Loading from "../../../utils/loadingPage";
import { useDispatch, useSelector } from "react-redux";
import { getItemSubCategories } from "../../../redux/slice";
...
const SettingsItemSubCategory = () => {
	...
	let tableName = SettingsItemSubCategoryUrl.split("/")
		.filter((part) => part.length > 0)
		.pop();
	let itemSubCategories = useSelector(
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsItemSubCategory/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getItemSubCategories(res));
		} else {
			let res = await CRUD("get", SettingsItemSubCategoryUrl, null, null);
			dispatch(getItemSubCategories(res));
		}
	};
	...
	const getItem = useCallback(async () => {
		...
		itemSubCategories && itemSubCategories.length > 0
			? setData(itemSubCategories)
			: setData([]);
		setLoading(false);
	}, [itemSubCategories, isLoading, loading]);
...
export default SettingsItemSubCategory;
