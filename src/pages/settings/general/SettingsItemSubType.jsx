...
const SettingsItemSubType = () => {
	...
	let tableName = SettingsItemSubTypeUrl.split("/")
		.filter((part) => part.length > 0)
		.pop();
	let itemSubTypes = useSelector((state) => state.settings.itemSubTypes);
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsItemSubType/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getItemSubTypes(res));
		} else {
			let res = await CRUD("get", SettingsItemSubTypeUrl, null, null);
			dispatch(getItemSubTypes(res));
		}
	};
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsItemSubTypeUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsItemSubTypeUrl, null, null);
					dispatch(getItemSubTypes(res));
				}
			} else {
				let result = await CRUD("post", SettingsItemSubTypeUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsItemSubTypeUrl, null, null);
					dispatch(getItemSubTypes(res));
				}
	...
	const getItem = useCallback(async () => {
		...
		itemSubTypes && itemSubTypes.length > 0
			? setData(itemSubTypes)
			: setData([]);
		setLoading(false);
	}, [itemSubTypes, isLoading, loading]);
...
export default SettingsItemSubType;
