...
import { SettingsLocationWarehouseUrl, BaseUrl } from "../../../utils/network";
import { CRUD, search, customRequest } from "../../../utils/js_functions";
import Loading from "../../../utils/loadingPage";
import { getWarehouses } from "../../../redux/slice";
...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationWarehouse = () => {
  ...
  let tableName = SettingsLocationWarehouseUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  const warehouses = useSelector((state) => state.settings.warehouses);
  let reports = useSelector((state) => state.settings.reports);
  let UOMs = useSelector((state) => state.settings.UOMs);
  let types = useSelector((state) => state.settings.warehouseTypes);
  let parentLocations = useSelector((state) => state.settings.branches);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationWarehouse/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getWarehouses(res));
    } else {
      let res = await CRUD("get", SettingsLocationWarehouseUrl, null, null);
      dispatch(getWarehouses(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    setLoading(true);
    warehouses && warehouses.length > 0 ? setData(warehouses) : setData([]);
    setLoading(false);
  }, [warehouses, isLoading, UOMs, types, parentLocations, reports]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record, index) => {
        ...
        return values.length > 0 ? values[0].code : "";
      },
    },
    {
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationWarehouse"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationWarehouse;
