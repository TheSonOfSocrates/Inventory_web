...
const SettingsItemType = () => {
	...
	let tableName = SettingsItemTypeUrl.split("/")
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsItemType/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getItemTypes(res));
		} else {
			let res = await CRUD("get", SettingsItemTypeUrl, null, null);
			dispatch(getItemTypes(res));
		}
	};
	...
	const handleOk = () => {
		form.validateFields().then(async (values) => {
			...
			if (editingItem) {
				setEditingItem(null);
				let result = await CRUD("put", SettingsItemTypeUrl, values, editId);
				if (result) {
					let res = await CRUD("get", SettingsItemTypeUrl, null, null);
					dispatch(getItemTypes(res));
				}
			} else {
				let result = await CRUD("post", SettingsItemTypeUrl, values, null);
				if (result) {
					let res = await CRUD("get", SettingsItemTypeUrl, null, null);
					dispatch(getItemTypes(res));
				}
	...
	const handleDelete = async (record) => {
		let result = await CRUD("delete", SettingsItemTypeUrl, record, record.id);
		if (result) {
			let res = await CRUD("get", SettingsItemTypeUrl, null, null);
			dispatch(getItemTypes(res));
		}
	...
	const getItem = useCallback(async () => {
		...
		itemTypes && itemTypes.length > 0 ? setData(itemTypes) : setData([]);
		setLoading(false);
	}, [itemTypes, isLoading, loading]);
	...
	const columns = [
		...
		},
		{
			...
			sortDirections: ["descend", "ascend"],
			render: (_, record, index) => {
				let res = accessCodes.find((item) => item.id === record.accessCode);
				return res?.name ?? "";
			},
		},
		{
