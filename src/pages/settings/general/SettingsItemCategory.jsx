...
const SettingsItemCategory = () => {
  ...
  let tableName = SettingsItemCategoryUrl.split("/")
  ...
  const onSearch = async (value) => {
    ...
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsItemCategory/?timestamp=${timestamp}&search=${value}`;
      res = await search("get", SearchUrl, null, null);
    } else {
      res = await CRUD("get", SettingsItemCategoryUrl, null, null);
    }
    dispatch(getItemCategories(res));
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsItemCategoryUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsItemCategoryUrl, null, null);
          dispatch(getItemCategories(res));
        }
      } else {
        let result = await CRUD("post", SettingsItemCategoryUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsItemCategoryUrl, null, null);
          dispatch(getItemCategories(res));
        }
  ...
  const getItem = useCallback(async () => {
    ...
    itemCategories && itemCategories.length > 0
      ...
      : setData([]);
    setLoading(false);
  }, [itemCategories, isLoading, loading]);
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ["descend", "ascend"],
      render: (_, record, index) => {
        let res = persons.find(
          (item) => item.id === record.person_id_updatedby
        );
        return res.length > 0 ? res.user_name : "";
      },
    },
  ];
...
export default SettingsItemCategory;
