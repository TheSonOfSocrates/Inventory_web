...
const SettingsLocationCountry = () => {
  ...
  let tableName = SettingsLocationCountryUrl.split("/")
  ...
  let initCountryTypes = useSelector((state) => state.settings.countryTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationCountry/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getCountries(res));
    } else {
      let res = await CRUD("get", SettingsLocationCountryUrl, null, null);
      dispatch(getCountries(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    countries && countries.length > 0 ? setData(countries) : setData([]);
    initReports && initReports.length > 0
      ? setReports(initReports)
      : setReports([]);
    uoms && uoms.length > 0 ? setUOMs(uoms) : setUOMs([]);
    initCountryTypes && initCountryTypes.length > 0
      ? setCountryTypes(initCountryTypes)
      : setCountryTypes([]);
    setLoading(false);
  }, [countries, initCountryTypes, uoms, initReports, isLoading, loading]);
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ["descend", "ascend"],
      render: (_, record, index) => {
        let r1 = countryTypes.filter((item) => item.id === record.type_id);
...
export default SettingsLocationCountry;
