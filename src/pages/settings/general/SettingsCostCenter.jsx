...
const SettingsCostCenter = () => {
  ...
  let tableName = SettingsCostCenterUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();
  let costCenters = useSelector((state) => state.settings.costCenters);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsCostCenter/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getCostCenters(res));
    } else {
      let res = await CRUD("get", SettingsCostCenterUrl, null, null);
      dispatch(getCostCenters(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsCostCenterUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsCostCenterUrl, null, null);
          dispatch(getCostCenters(res));
        } else return result;
      } else {
        let result = await CRUD("post", SettingsCostCenterUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsCostCenterUrl, null, null);
          dispatch(getCostCenters(res));
        } else return result;
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsCostCenterUrl, record, record.id);
    if (result) {
      let res = await CRUD("get", SettingsCostCenterUrl, null, null);
      dispatch(getCostCenters(res));
    } else return result;
  ...
  const getItem = useCallback(async () => {
    ...
    costCenters && costCenters.length > 0 ? setData(costCenters) : setData([]);
    setLoading(false);
  }, [costCenters, isLoading, loading]);
...
export default SettingsCostCenter;
