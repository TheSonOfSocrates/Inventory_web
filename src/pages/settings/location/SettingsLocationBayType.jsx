...
const SettingsLocationBayType = () => {
	...
	let tableName = SettingsLocationBayTypeUrl.split("/")
		.filter((part) => part.length > 0)
		.pop();

	let bayTypes = useSelector((state) => state.settings.bayTypes);
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsLocationBayType/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getBayTypes(res));
		} else {
			let res = await CRUD("get", SettingsLocationBayTypeUrl, null, null);
			dispatch(getBayTypes(res));
		}
	};
	...
	const getItem = useCallback(async () => {
		...
		bayTypes && bayTypes.length > 0 ? setData(bayTypes) : setData([]);

		setLoading(false);
	}, [bayTypes, isLoading, loading]);
...
export default SettingsLocationBayType;
