...
const SettingsLocationRegion = () => {
  ...
  let tableName = SettingsLocationRegionUrl.split("/")
  ...
  let initTypes = useSelector((state) => state.settings.regionTypes);
  let initParentLocations = useSelector((state) => state.settings.countries);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationRegion/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getRegions(res));
    } else {
      let res = await CRUD("get", SettingsLocationRegionUrl, null, null);
      dispatch(getRegions(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    regions && regions.length > 0 ? setData(regions) : setData([]);
...
export default SettingsLocationRegion;
