...
const SettingsLocationRoom = () => {
  ...
  let tableName = SettingsLocationRoomUrl.split("/")
  ...
  let initTypes = useSelector((state) => state.settings.roomTypes);
  let initParentLocations = useSelector((state) => state.settings.areas);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationRoom/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getRooms(res));
    } else {
      let res = await CRUD("get", SettingsLocationRoomUrl, null, null);
      dispatch(getRooms(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsLocationRoomUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsLocationRoomUrl, null, null);
          dispatch(getRooms(res));
        }
      } else {
        let result = await CRUD("post", SettingsLocationRoomUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsLocationRoomUrl, null, null);
          dispatch(getRooms(res));
        }
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsLocationRoomUrl, null, record.id);
    if (result) {
      let res = await CRUD("get", SettingsLocationRoomUrl, null, null);
      dispatch(getRooms(res));
    }
  ...
  const getItem = useCallback(async () => {
    ...
    rooms && rooms.length > 0 ? setData(rooms) : setData([]);
...
export default SettingsLocationRoom;
