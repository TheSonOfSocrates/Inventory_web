...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationAreaType = () => {
	...
	let tableName = SettingsLocationAreaTypeUrl.split("/")
		.filter((part) => part.length > 0)
		.pop();

	let areaTypes = useSelector((state) => state.settings.areaTypes);
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/SettingsLocationAreaType/?timestamp=${timestamp}&search=${value}`;
			let res = await search("get", SearchUrl, null, null);
			dispatch(getAreaTypes(res));
		} else {
			let res = await CRUD("get", SettingsLocationAreaTypeUrl, null, null);
			dispatch(getAreaTypes(res));
		}
	};
	...
	const getItem = useCallback(async () => {
		...
		areaTypes && areaTypes.length > 0 ? setData(areaTypes) : setData([]);
		setLoading(false);
	}, [areaTypes, isLoading, loading]);
	...
	return (
		<div>
			<PageLayout
				title="SettingsLocationAreaType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationAreaType;
