...
import { SettingsCurrencyUrl, BaseUrl, SettingsTransferTypeUrl } from "../../../utils/network";
import { CRUD, search, customRequest } from "../../../utils/js_functions";
import Loading from "../../../utils/loadingPage";
import { useDispatch, useSelector } from "react-redux";
import {  getTransferTypes } from "../../../redux/slice";
...
const SettingsTransferType = () => {
  ...
  let tableName = SettingsTransferTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();
    
  let transferTypes = useSelector((state) => state.settings.transferTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsTransferType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getTransferTypes(res));
    } else {
      let res = await CRUD("get", SettingsTransferTypeUrl, null, null);
      dispatch(getTransferTypes(res));
    }
  };
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD("put", SettingsTransferTypeUrl, values, editId);
        if (result) {
          let res = await CRUD("get", SettingsTransferTypeUrl, null, null);
          dispatch(getTransferTypes(res));
        } else return result;
      } else {
        let result = await CRUD("post", SettingsTransferTypeUrl, values, null);
        if (result) {
          let res = await CRUD("get", SettingsTransferTypeUrl, null, null);
          dispatch(getTransferTypes(res));
        } else return result;
  ...
  const handleDelete = async (record) => {
    let result = await CRUD("delete", SettingsTransferTypeUrl, record, record.id);
    if (result) {
      let res = await CRUD("get", SettingsTransferTypeUrl, null, null);
      dispatch(getTransferTypes(res));
    } else return result;
  ...
  const getItem = useCallback(async () => {
    ...
    transferTypes && transferTypes.length > 0 ? setData(transferTypes) : setData([]);
    setLoading(false);
  }, [transferTypes, isLoading, loading]);
...
export default SettingsTransferType;
